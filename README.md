...
No build step, no dependencies, no backend. Open `simulator/index.html` and it runs.

---

## Running it

**Hosted, behind accounts (what the site runs on):** deploy to Cloudflare Pages
with a build output directory of `public/`. The landing page at `/` stays open to
everybody; `/simulator/` and every file it needs require an account. The gate is
`functions/_middleware.js` and it runs server-side, before any file is handed
over. Setup — Supabase, environment variables, the domain move — is in
[`docs/AUTH-SETUP.md`](docs/AUTH-SETUP.md).

This cannot run on GitHub Pages. Pages serves static files and executes no code,
so the gate would never run and the simulator would simply be public.

**Local:** clone the repository and open `public/simulator/index.html` in any modern
browser (or `public/index.html` for the landing page). It works straight off the
filesystem — no server required, and no login, since there is no server to check
one — so a preceptor can carry the folder on a USB stick.
...
The login flow itself needs the Functions runtime, so to exercise that locally
you want `npx wrangler pages dev public` with the environment variables set.

**Offline single file:** `npm run standalone` bundles the simulator into
`12-lead-generator.html` — no server, no network, and **no gate**. Useful for
teaching without wifi; do not put it anywhere public.
...
# -> public/assets/data/ptbxl-profiles.json   (a few hundred KB)
```
...
```
functions/            Cloudflare Pages Functions — the account gate and its API
  _middleware.js      runs before every request; decides public vs. gated
  api/                signup, login, logout, resend, recover, reset, me
  auth/confirm.js     where the links in the confirmation emails land
lib/                  session signing, cookies, the Supabase client
tests/                npm test — the gate, sessions, and the account flows
public/               everything Cloudflare deploys
  index.html            public landing page
  login.html            sign in / sign up / forgot password
  reset.html            set a new password
  simulator/index.html  the app itself — gated
  assets/css/app.css
  assets/css/landing.css
  assets/css/auth.css
  assets/img/           tracings used on the landing page, generated by the app
  assets/js/
    ...
    account.js      the signed-in strip in the header
```
