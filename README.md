...
(Settings → Pages → deploy from branch, root folder). The root serves the public
landing page and `/simulator/` serves the app, so students only need the site URL.

**Local:** clone the repository and open `simulator/index.html` in any modern browser
(or `index.html` for the landing page). It works straight off the filesystem — no
server required — so a preceptor can carry the folder on a USB stick.
...
simulator/index.html  the app itself
assets/css/app.css
