 ...
 * `https://<ref>.supabase.co/rest/v1/` — on the screen where somebody goes
 * looking for "the project URL", so that is what gets pasted. Appending
 * `/auth/v1/token` to it produces a 404 that surfaces as a failed sign-in
 * rather than as a wrong address, which is a miserable thing to debug.
 *
 * Only the service prefixes are trimmed, not any path, so a self-hosted
 * instance living under a subdirectory still works. */
const API_SUFFIX = /\/(rest|auth|storage|realtime|functions)\/v\d+$/;
...
export function config(env) {
  ...
  return {
    url: projectUrl(env.SUPABASE_URL),
