...
The app itself has not changed. What changed is where it is served from: it now
sits behind a gate that checks for a session before it will hand over the page
or any of its scripts.
...
1. Visit the site — you should land on `/login`.
2. Ask for `https://your-site/assets/js/explain.js` directly. You should get a
...
3. Create an account. You should be told to check your email, and *not* be
   signed in.
4. Try to sign in before clicking the link — it should refuse and offer to
   resend the email.
5. Click the confirmation link. You should land in the simulator, signed in.
6. Sign out, run through "Forgot your password?", and set a new one.
...
public/               everything here is private by default
